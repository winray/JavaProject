...
public class Jigsaw {
	...
	private void estimateValue(JigsawNode jNode) {
		...
		for (int index = 1; index < dimension * (dimension - 1); index++) {
			if ((index-1) / dimension == dimension-1)
				continue;
			if (jNode.getNodesState()[index] == 0)
				continue;
			// 纵向后续节点不正确
            ...
            if(jNode.getNodesState()[index] != endJNode.getNodesState()[index]) {
                for (int j = 1; j != dimension * dimension; j++) {
                    if (jNode.getNodesState()[index] == endJNode.getNodesState()[j]) {
                        int rowx = (index - 1) / dimension;
                        int colx = (index - 1) % dimension;
                        int rowy = (j - 1) / dimension;
                        int coly = (j - 1) % dimension;
                        //曼哈顿距离
                        s1 += Math.abs(rowx - rowy) + Math.abs(colx - coly);
                        //欧几理德距离的平方
                        s2 += Math.pow((rowx - rowy),2) + Math.pow((colx - coly),2);
